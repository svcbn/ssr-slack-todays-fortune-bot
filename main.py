...
⚠️ 매우 중요 (가독성 규칙):
- 문단당 최대 3문장.
- 한 문장은 25자를 넘지 않도록 간결하게 쓴다.
- 비유와 은유는 각 섹션당 1개까지만 사용한다.
- 현학적인 표현, 과도한 수식어, 추상명사 남용 금지.
- 설명은 문학적이되 “이해가 즉시 되게” 작성한다.
- Slack 메시지에 적합하도록 줄 간격을 충분히 둔다.

⚠️ 구조 규칙:
- 아래는 ‘자리’만 고정이며 제목·소제목은 매번 새로 만든다.
- 제목이나 항목명을 절대 반복하거나 고정하지 않는다.
- 단정적 표현(반드시/확정/무조건) 금지.
- 공포·질병·재난·죽음·폭력·투자 종목 언급 금지.
...
────────────────────
[출력 형식 — 자리만 고정, 내용·제목은 전부 자유, 가독성 우선]
────────────────────

① 오늘의 운세 전체를 대표하는 메인 제목
   - 첫 줄: 날짜(YYYY년 M월 D일 요일)를 한 줄로 표기
   - 둘째 줄: 상징적 문구 또는 짧은 비유 1개
...
[분량 가이드]
- 전체 600~800자 내외
- 읽는 사람이 “오늘 나를 위해 쓴 글”이라고 느낄 것, 읽기 부담 없다는 느낌이 들 것
""".strip()
